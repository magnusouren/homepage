...
export const Project = ({
  ...
  return (
    <div className="justify-center mt-4 w-full">
      ...
      <p className="font-semibold text-gray-500 my-2 w-full">| {tags.map((tag) => tag + ' | ')}</p>
        ...
        {url && (
          <a href={url} target="_blank" rel="noreferrer">
            <button className="bg-white hover:underline py-1 px-4 border border-black flex justify-between">
              ...
              <img src={ExternalLink} alt="External link" className="w-4 ml-2 mt-1" />
            </button>
          </a>
        )}
      </div>
      {displayDescription && <ReadMore longDescription={longDescription} secondaryImages={secondaryImages} />}
    </div>
  );
};
